...
import protobuf.messageManager.handler.NetWorkGameHandler;
...
public class GameLevelHandler{
    ...
    //根据mapNum的位数来判断是第几关
    public void gameLevelHandler(TankWorld oldTankWorld,int groupNum,int mapNum,ArrayList<ChannelHandlerContext> channelHandlerContextArrayList){
        ...
        if(mapNum>=0&&mapNum<10){//第一关完成了，进入第二关
            ...
            NetWorkGameHandler.tankWorlds.remove(oldTankWorld);
            NetWorkGameHandler.tankWorlds.add(tankWorld);
